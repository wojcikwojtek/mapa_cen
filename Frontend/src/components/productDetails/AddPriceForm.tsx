...
const AddPriceForm = ({ closeForm, productId, selectedRegion, update }: Props) => {
  ...
  const onSubmit = (data: FieldValues) => {
    console.log("wysylanie");
    apiClient.addNewPrice(productId, selectedRegion, data.address, parseFloat(data.price))
      .then(res => {
        console.log(res.data);
        closeForm();
        update();
  ...
  return (
    <div className='addPriceForm'>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <label htmlFor="address">Adres sklepu:</label><br />
        <input {...register('address', { required: true, minLength: 5 })} type="text" id="address" name="address" /><br />
        <label htmlFor="priceValue">Cena:</label><br />
        <input {...register('price', { required: true, min: 0.01 })} type="number" id="price" name="price" step="0.01" /><br />
        {errors.address && <p>zły adres</p>}
        {errors.price && <p>zła cena</p>}
        <input type="submit" value="Dodaj cenę" />
      </form>
    </div>
  )
}
